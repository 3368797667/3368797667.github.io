...
def handle_join(data):
    ...
    if len(rooms[room]['players']) == 2:
        ...
        init_board(room)
        # 通知双方游戏开始
        socketio.emit('game_start', rooms[room]['board'], room=room)
...
def handle_move(data):
    ...
    # 验证移动合法性
    if validate_move(rooms[room]['board'], from_pos, to_pos):
        # 更新棋盘状态
        rooms[room]['board'][to_pos[0]][to_pos[1]] = rooms[room]['board'][from_pos[0]][from_pos[1]]
        rooms[room]['board'][from_pos[0]][from_pos[1]] = 0
        # 广播新棋盘状态
        socketio.emit('update_board', rooms[room]['board'], room=room)
...
def handle_move(data):
    ...
    socketio.emit('update_board', {
        'board': board,
        'current_player': rooms[room].get('current_player', 1)
    }, room=room)
...
# 修改房间初始化逻辑
def init_board(room):
    rooms[room]['board'] = get_initial_board()
    rooms[room]['current_player'] = 1  # 红方先手